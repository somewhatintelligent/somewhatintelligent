 ...
 */
export const settle = Effect.fn("Settlement.settle")(
  function* (
    event: ProviderEvent,
    attempt: number,
    livemode: boolean,
  ...
  /**
   * Put what happened to the money ON the span.
   *
   * The span already existed; what it could not tell you is whether a webhook
   * settled an order, was a replay, or was dropped by the environment gate —
   * those are the same span, same duration, same OK status. `outcome` is the only
   * thing separating "we took payment and recorded it" from "we silently ignored
   * a live event", so it belongs here rather than in a log line someone has to
   * go and correlate afterwards.
   */
  (effect) =>
    Effect.tap(effect, (settlement) =>
      Effect.annotateCurrentSpan({
        "commerce.settlement.outcome": settlement.outcome,
        ...(settlement.orderNumber === null
          ? {}
          : { "commerce.order.number": settlement.orderNumber }),
      }),
    ),
);
