...
test(
  ...
  Effect.gen(function* () {
     ...
     * INSERTS at the origin root. Clients differ on which they ask for, so all
     * three have to answer — and the third is the one `app/worker.ts` dropped.
     *
     * Concurrently: three unrelated GETs of the same static document. Against a
     * real deploy (`ALCHEMY_DEV=0`) they then ride out ONE cold-start window
     * between them rather than three in series.
     */
    yield* Effect.forEach(
      [DISCOVERY, `${AUTH_BASE_PATH}/.well-known/oauth-authorization-server`, OAUTH_METADATA_PATH],
      (path) =>
...
test(
  ...
  Effect.gen(function* () {
     ...
     *
     * `test/scopes.test.ts` pins the same thing against the resolved options,
     * so the cheap suite catches it first; this is the end-to-end confirmation.
     */
...
/**
 * THE POSTURE, and the one assertion that decides it. Why it is 401 and not
 * 201 is argued where the decision is made — see the
 * `allowDynamicClientRegistration` note in `api/config.ts`, next to the flag
 * that is deliberately absent beside it.
 */
test(
  "an anonymous caller cannot, which is a decision and not a default",
  Effect.gen(function* () {
    const { origin } = yield* stack;
