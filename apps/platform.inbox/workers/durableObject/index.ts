...
export class MailboxDO extends DurableObject<Env> {
  ...
  constructor(state: DurableObjectState, env: Env) {
    super(state, env);
    /**
     * `{ schema }` is the drizzle 0.45 shape, and the reason this package pins
     * `drizzle-orm` to `^0.45.2` rather than the workspace catalog's
     * `1.0.0-rc.4` — 1.0 replaced it with `relations` and typechecks this call
     * as an unknown property. platform.auth's D1 is on the catalog version;
     * two majors coexist and bun nests this one under the package.
     *
     * Moving to 1.0 means reworking every query below against a storage layer
     * that has no tests, so it is deliberately not part of the port.
     */
