...
app.post("/api/v1/mailboxes/:mailboxId/emails", async (c: AppContext) => {
  ...
  const { in_reply_to, references, thread_id } = body;

  return sendFromMailbox(c, body, {
    threading: {
      inReplyTo: in_reply_to ?? null,
      references: references ?? [],
      // The caller may name the thread; otherwise a message answering another
      // joins that one's, and a message answering nothing opens its own.
