 ...
 *   - generated/email/tokens.ts       (inlined hex — mail clients have no var())
 *
 * Run: bun run codegen
 */
...
await mkdir(`${GENERATED_DIR}/email`, { recursive: true });
...
function generateTailwindTheme(): string {
  ...
  return [
    ...
    '   @import "tailwindcss";',
    '   @import "platform.design/generated/css/tokens.css";',
    '   @import "platform.design/generated/css/tailwind-theme.css";',
    "   ═══════════════════════════════════════════════════ */",
    "",
    "@theme inline {",
    ...colorLines,
...
/**
 * Email is the one target that CANNOT read CSS custom properties — clients
 * strip `<style>` and none of them resolve `var()`. Every value has to be
 * inlined at the point of use, which is exactly the condition under which a
 * hand-maintained copy rots: the previous one drifted four forks from this
 * package while its header comment still claimed to mirror it.
 *
 * So it is generated. Retinting `brand.ts` retints email, `audit-contrast.ts`
 * already covers these values, and `brand-lint` keeps the hex from being
 * written by hand anywhere else.
 *
 * LIGHT ONLY. `prefers-color-scheme` support across mail clients is partial and
 * inconsistent, and a half-applied dark theme is worse than none — cold paper
 * is the ground, as on the sign-in surface.
 */
function generateEmailTokens(): string {
  const lines: string[] = [
  ...
  for (const key of SEMANTIC_KEYS) {
    lines.push(`  ${key}: "${lightColors[key].hex}",`);
  }
  lines.push("} as const;", "");
  ...
  for (const [key, stack] of Object.entries(fontStacks)) {
    lines.push(`  ${key}: ${JSON.stringify(stack.family)},`);
  }
  lines.push("} as const;", "");
  ...
  // No trailing blank line: the formatter strips it, which would leave
  // `bun run codegen` and `vp check --fix` overwriting each other forever.
  return `${lines.join("\n")}\n`;
}
...
const outputs = [
  ...
  { path: "email/tokens.ts", content: generateEmailTokens() },
];
