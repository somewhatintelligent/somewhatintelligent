...
Component library for the platform, built on [Base UI](https://base-ui.com) primitives and styled per the design system in `packages/platform.design/DESIGN_SYSTEM.md`.
...
All components are consumed via the `platform.ui` package alias. The export map supports three entry points:

| Pattern                    | Resolves to               | Example                                |
| -------------------------- | ------------------------- | -------------------------------------- |
| `platform.ui`              | `src/components/index.ts` | Barrel re-export of core UI primitives |
| `platform.ui/components/*` | `src/components/ui/*.tsx` | `platform.ui/components/button`        |
| `platform.ui/lib/*`        | `src/lib/*.ts`            | `platform.ui/lib/utils`                |

Form components and hooks are imported directly:

```ts
import { useAppForm } from "platform.ui/hooks/use-app-form";
import { TextField } from "platform.ui/components/form/text-field";
```
...
Every component follows the design system defined in `packages/platform.design/DESIGN_SYSTEM.md`. The key principles:
...
```ts
import { surfaceMaterials, interactiveMaterials } from "platform.ui/lib/materials";
```
...
```tsx
import { useAppForm, withForm } from "platform.ui/hooks/use-app-form";
...
```tsx
import { withFieldGroup } from "platform.ui/hooks/use-app-form";
...
| ---------------------- | --------------------------- | ---------------------------------------------------------- |
| `cn(...classes)`       | `platform.ui/lib/utils`     | Merges class names via `clsx` + `tailwind-merge`           |
| `surfaceMaterials`     | `platform.ui/lib/materials` | Static surface class strings (brutal, soft, neo, glass)    |
| `interactiveMaterials` | `platform.ui/lib/materials` | Interactive element class strings with hover/active states |
| `compactMaterials`     | `platform.ui/lib/materials` | Compact element class strings (badges)                     |
