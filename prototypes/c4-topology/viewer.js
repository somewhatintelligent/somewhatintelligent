...
const ISSUE_KINDS = {
  violation: { label: "boundary violation", severity: "critical" },
  cycle: { label: "import cycle", severity: "critical" },
  hotspot: { label: "complexity hotspot", severity: "critical" },
  dead: { label: "dead export", severity: "warn" },
  orphan: { label: "unreferenced file", severity: "warn" },
  complex: { label: "complex function", severity: "warn" },
};
...
const fileIssues = (f) => {
  ...
  if (f.max_cyclomatic >= HOTSPOT_AT) counts.hotspot = 1;
  else if (f.max_cyclomatic >= COMPLEX_AT) counts.complex = 1;
  return counts;
};
...
for (const [key, container] of Object.entries(CODE.containers)) {
  const own = container.components.filter((c) => !c.pkg).flatMap((c) => c.files);
...
const worstSeverity = (counts) =>
  Object.keys(counts).some((k) => ISSUE_KINDS[k]?.severity === "critical")
    ? "critical"
...
const viewFiles = (view) => {
  ...
  if (view.codeKey) {
    return new Set(
      CODE.containers[view.codeKey].components.filter((c) => !c.pkg).flatMap((c) => c.files),
    );
  }
  if (view.id.startsWith("codefiles:")) {
    const [, key, ...rest] = view.id.split(":");
    const comp = CODE.containers[key]?.components.find((c) => c.id === rest.join(":"));
    return new Set(comp?.files ?? []);
  }
  return new Set();
};
...
const chipsFor = (counts) =>
  Object.entries(counts)
    .sort(
      (a, b) =>
        (ISSUE_KINDS[a[0]].severity === "critical" ? -1 : 1) -
        (ISSUE_KINDS[b[0]].severity === "critical" ? -1 : 1),
    )
    .map(([kind, n]) => ({
      kind,
      n,
      severity: ISSUE_KINDS[kind].severity,
      short: `${n} ${kind === "dead" ? "dead" : kind === "orphan" ? "orphan" : kind === "violation" ? "breach" : kind}`,
...
const views = [
  ...
  ...codeKeys.map((key) => ({
    ...
    codeKey: key,
    title: `Modules — ${nodeByKey.get(key)?.name ?? key} (${CODE.containers[key].appRoot})`,
  })),
  {
    id: "components",
...
const resolveView = (id) => {
  ...
  if (id.startsWith("codefiles:")) {
    ...
    return {
      ...
      label: comp,
      parent: "code:" + key,
      title: `Files — ${nodeByKey.get(key)?.name ?? key} / ${comp}`,
    };
  }
  return views[0];
};
...
function systemGraph(stack) {
  ...
  const nodes = [...memberKeys].map((key) => {
    ...
    return {
      ...
      member: true,
      chips: chipsFor(issues),
      drill: CODE.containers[key] ? "code:" + key : undefined,
    };
  });
...
function componentsGraph() {
  ...
  return {
    ...
    boundary: { label: "Commerce Core — hexagonal interior (hand-authored)" },
  };
}
...
function codeCompGraph(key) {
  ...
  const nodes = container.components.map((c) => {
    const files = c.files.map((i) => CODE.files[i]);
    ...
    return {
      ...
      member: !c.pkg,
      chips: chipsFor(issuesOfFiles(c.files)),
    };
  });
  ...
  const edges = container.edges.map((e) => {
    ...
    return {
      ...
      label: violation
        ? "breaks boundary"
        : `${e.n} import${e.n === 1 ? "" : "s"}${e.typeOnly ? " · types" : ""}`,
  ...
  return {
    ...
    edges,
    boundary: {
      label: `${nodeByKey.get(key)?.name ?? key} · ${container.appRoot}`,
      memberOnly: true,
    },
  };
}
...
function codeFileGraph(key, compId) {
  ...
  for (const [from, to, flags] of CODE.edges) {
    ...
    if (fromIn && toIn) raw.push({ from: `codefile:${from}`, to: `codefile:${to}`, typeOnly });
    else if (fromIn) {
      neighbourComps.add(compOfFile.get(to));
      raw.push({ from: `codefile:${from}`, to: `codecomp:${key}:${compOfFile.get(to)}`, typeOnly });
    } else {
      neighbourComps.add(compOfFile.get(from));
      raw.push({ from: `codecomp:${key}:${compOfFile.get(from)}`, to: `codefile:${to}`, typeOnly });
    }
  }

  const fileIds = [...member].map((i) => `codefile:${i}`);
  const cols = layerByDepth(
    fileIds,
  ...
  for (const compName of neighbourComps) {
    nodes.push({
      ...
      kindLine: "[module]",
      compact: true,
      col: raw.some((e) => e.from === `codecomp:${key}:${compName}`) ? 0 : 4,
  ...
  return {
    nodes,
    edges,
    boundary: { label: `${nodeByKey.get(key)?.name ?? key} / ${compId}`, memberOnly: true },
  };
}
...
const graphFor = (id) =>
  id === "landscape"
    ...
    : id === "components"
      ? componentsGraph()
      : id.startsWith("code:")
        ? codeCompGraph(id.slice(5))
        : id.startsWith("codefiles:")
          ? codeFileGraph(id.split(":")[1], id.split(":").slice(2).join(":"))
          : systemGraph(id.slice(7));
...
function canvas(graph, view) {
  ...
  labelsOnHoverOnly = graph.edges.length > 14;
  ...
  root.append(edgeLayer(graph, placed));
 ...
 */
function edgeLayer(graph, placed) {
  const layer = svgEl("g", { class: labelsOnHoverOnly ? "edges quiet" : "edges" });
  const descs = [];
  ...
  let loopNth = 0;
  for (const d of descs) layer.append(edgeEl(d, d.same ? loopNth++ : 0));
  return layer;
}
...
function nodeEl(n) {
  const severity = n.chips?.length
    ? n.chips.some((c) => c.severity === "critical")
      ? "critical"
      : "warn"
    : null;
  const g = svgEl("g", {
  ...
  if (n.chips?.length) {
    ...
    for (const chip of n.chips.slice(0, 3)) {
      ...
      g.append(
        ...
        svgEl(
          "text",
          { class: "chiptext", x: x + w / 2, y: n.y + n.h - 7, "text-anchor": "middle" },
...
function focusNode(key, on) {
  svg.querySelector(".edges")?.classList.toggle("focusing", on);
  for (const edge of svg.querySelectorAll(".edge")) {
    ...
    if (!edge.classList.contains("violation")) {
      edge
        .querySelector("path")
        .setAttribute("marker-end", on && touches ? "url(#arr-hot)" : "url(#arr)");
    }
  }
  const connected = new Set([key]);
  if (on) {
    for (const edge of svg.querySelectorAll(".edge")) {
      if (edge.dataset.from === key) connected.add(edge.dataset.to);
      if (edge.dataset.to === key) connected.add(edge.dataset.from);
    }
  }
  for (const node of svg.querySelectorAll(".node"))
    node.classList.toggle("dim", on && !connected.has(node.dataset.key));
}

/** Clicking a stat chip pins every node carrying that issue. */
function highlightIssue(kind) {
  const wanted = new Set(
    [...svg.querySelectorAll(".node")]
      .filter((el) => el.getAttribute("aria-label")?.includes(ISSUE_KINDS[kind].label))
      .map((el) => el.dataset.key),
  );
  const already = svg.classList.contains("filtering") && svg.dataset.issue === kind;
  svg.classList.toggle("filtering", !already);
  svg.dataset.issue = already ? "" : kind;
  for (const node of svg.querySelectorAll(".node"))
    node.classList.toggle("dim", !already && !wanted.has(node.dataset.key));
}
    ...
    for (const e of list) {
      ...
      ul.append(
        el(
          ...
          el("div", {
            class: "who",
            text: (dir === "out" ? "→ " : "← ") + (nodeByKey.get(otherKey)?.name ?? otherKey),
          }),
          el("div", {
            class: "lbl",
...
const searchIndex = (() => {
  ...
  return [...new Map(entries.map((e) => [e.select ?? `${e.view}|${e.label}`, e])).values()];
})();
...
function searchOverlay() {
  ...
  const run = () => {
    ...
    const matches = (
      query
        ? searchIndex.filter(
            (e) => e.label.toLowerCase().includes(query) || e.hint.toLowerCase().includes(query),
          )
...
function attachPanZoom() {
  ...
  svg.addEventListener("click", (ev) => {
    if (ev.target.closest(".node")) return;
    if (svg.classList.contains("filtering")) return highlightIssue(svg.dataset.issue);
    if (selectedKey) {
      selectedKey = null;
      render();
    }
